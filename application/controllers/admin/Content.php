...
class Content extends Admin_Controller
{
    ...
    public function download_student_application($id)
    {
        $this->load->helper('file'); // Load file helper
        $filename='user_'.$id.'_application.pdf';
        $content = $this->uploadcontent_model->application_download($id);
        $this->media_storage->applicationfiledownload($filename, $content->file_path);
    }


    public function view_pdf($student_id)
    {
        // Load any necessary libraries or helpers for working with files and paths
        $this->load->helper('file');

        // Generate the path to the PDF file
        $content = $this->uploadcontent_model->application_download($student_id);

        $path = $this->media_storage->applicationfileviewpath($content->file_path);
        ...
        if (file_exists($path)) {
            ...
            show_error('The requested PDF file does not exist.', 404);
        }
    }
