...
class Feediscount extends Admin_Controller
{
    ...
    public function assign($id)
    {
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            $resultlist          = $this->feediscount_model->searchAssignFeeByClassSectionapproval($data['class_id'], $data['section_id'], $id, $data['category_id'], $data['gender'], $data['rte_status']);
    ...
    public function studentdiscount()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (!empty($diff_aray)) {
                $this->feediscount_model->deletedisstdapproval($feediscount_id, $diff_aray);
            }
