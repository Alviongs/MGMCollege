...
class Examtype extends Admin_Controller
{
    ...
    {
        if (!$this->rbac->hasPrivilege('internal_result_examtype', 'can_view')) {
            access_denied();
        }
        $this->session->set_userdata('top_menu', 'Student Information');
        $this->session->set_userdata('sub_menu', 'category/index');
        $data['title']        = $this->lang->line('category_list');
        $category_result      = $this->examtype_model->get();
        $data['categorylist'] = $category_result;
        $this->load->view('layout/header', $data);
        $this->load->view('admin/results/resulttype', $data);
    ...
    public function view($id)
    {
        ...
        $data['title']    = $this->lang->line('category_list');
        $category         = $this->examtype_model->get($id);
    ...
    {
        if (!$this->rbac->hasPrivilege('internal_result_examtype', 'can_delete')) {
            access_denied();
        }
        $data['title'] = $this->lang->line('category_list');
        ...
        redirect('admin/results/examtype/index');
    }

    public function create()
    {
        if (!$this->rbac->hasPrivilege('internal_result_examtype', 'can_add')) {
            access_denied();
        }
    ...
    {
        if (!$this->rbac->hasPrivilege('internal_result_examtype', 'can_edit')) {
            access_denied();
        }
        ...
        if ($this->form_validation->run() == false) {
            $this->load->view('layout/header', $data);
            $this->load->view('admin/results/resulttypeedit', $data);
            $this->load->view('layout/footer', $data);
        } else {
            $data = array(
                'id'       => $id,
