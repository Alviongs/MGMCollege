...
class Accountcategory_model extends MY_Model
{
    ...
    {
        $this->db->select()->from('accountcategory');
    ...
    public function check_data_exists($name, $id)
    {
        ...
        $this->db->where('id !=', $id);
        $query = $this->db->get('accountcategory');
        if ($query->num_rows() > 0) {
            return true;
        } else {
            return false;
