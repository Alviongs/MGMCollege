...
class Feegroupadding_model extends MY_Model
{
    ...
    public function remove($id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->where('is_system', 0);
        $this->db->delete('fee_groupsadding');
    ...
    public function check_data_exists($name, $id)
    {
        ...
        $this->db->where('id !=', $id);
        $query = $this->db->get('fee_groupsadding');
        if ($query->num_rows() > 0) {
            return true;
        } else {
            return false;
