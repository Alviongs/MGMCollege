...
class Feetypeadding_model extends MY_Model
{
    ...
    {
        $this->db->select()->from('feetypeadding');
    ...
    public function add($data)
    {
        ...
        if (isset($data['id'])) {
            ...
            $this->db->update('feetypeadding', $data);
            $message   = UPDATE_RECORD_CONSTANT . " On  fee type id " . $data['id'];
            $action    = "Update";
            $record_id = $data['id'];
            $this->log($message, $record_id, $action);
        } else {
            $this->db->insert('feetypeadding', $data);
    ...
    public function check_data_exists($name, $id)
    {
        ...
        $this->db->where('id !=', $id);
        $query = $this->db->get('feetypeadding');
        if ($query->num_rows() > 0) {
            return true;
        } else {
            return false;
