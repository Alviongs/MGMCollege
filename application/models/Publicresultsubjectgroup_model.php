...
class Publicresultsubjectgroup_model extends MY_Model {
     ...
     */
     public function getadstudentid($admino){
        $this->db->select('id')->from('students')
                 ->where('students.admission_no',$admino);
    ...
    public function getadstudentresults($adminno,$resultgrp,$sessionid){

        $this->db->select()->from('internalresulttable');
        $this->db->join('students','students.id=internalresulttable.stid');
